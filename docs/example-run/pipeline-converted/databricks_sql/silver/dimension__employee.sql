-- Source: DW:Dimension.Employee  (/Users/shivaiyer/Documents/Claude/sql-server-samples/samples/databases/wide-world-importers/wwi-dw-ssdt/wwi-dw-ssdt/Dimension/Tables/Employee.sql)
-- Converted: TABLE -> Delta table. Semantics preserved; SQL Server-specific DDL clauses
-- (IDENTITY, system-versioning, MEMORY_OPTIMIZED, partition schemes) translated or
-- flagged below rather than silently dropped.

CREATE TABLE IF NOT EXISTS wwi_<env>.silver.employee (
    Employee STRING NOT NULL COMMENT 'source type: Key]',
...
USING DELTA
COMMENT 'Converted from DW:Dimension.Employee'
TBLPROPERTIES (
  'delta.enableChangeDataFeed' = 'true'
);

-- Conversion notes:
-- Column `Employee` (Key] -> STRING): Unrecognised SQL Server type 'Key]' — defaulted to STRING.
