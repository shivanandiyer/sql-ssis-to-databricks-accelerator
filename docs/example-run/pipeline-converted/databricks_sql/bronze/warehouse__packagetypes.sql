-- Source: OLTP:Warehouse.PackageTypes  (/Users/shivaiyer/Documents/Claude/sql-server-samples/samples/databases/wide-world-importers/wwi-ssdt/wwi-ssdt/Warehouse/Tables/PackageTypes.sql)
-- Converted: TABLE -> Delta table. Semantics preserved; SQL Server-specific DDL clauses
-- (IDENTITY, system-versioning, MEMORY_OPTIMIZED, partition schemes) translated or
-- flagged below rather than silently dropped.

CREATE TABLE IF NOT EXISTS wwi_<env>.bronze.warehouse__packagetypes (
    PackageTypeID INT NOT NULL,
    PackageTypeName STRING NOT NULL,
    LastEditedBy INT NOT NULL
)
USING DELTA
COMMENT 'Converted from OLTP:Warehouse.PackageTypes'
TBLPROPERTIES (
  'delta.enableChangeDataFeed' = 'true'
);

-- Conversion notes:
-- Column `PackageTypeID` used IDENTITY/NEXT VALUE FOR in the source — replace with a Delta `GENERATED
...
-- [FK_Warehouse_PackageTypes_Application_People] FOREIGN KEY ([LastEditedBy]) REFERENCES
-- [Application].[People]
