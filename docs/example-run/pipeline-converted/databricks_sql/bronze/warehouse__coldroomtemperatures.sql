-- Source: OLTP:Warehouse.ColdRoomTemperatures  (/Users/shivaiyer/Documents/Claude/sql-server-samples/samples/databases/wide-world-importers/wwi-ssdt/wwi-ssdt/Warehouse/Tables/ColdRoomTemperatures.sql)
-- Converted: TABLE -> Delta table. Semantics preserved; SQL Server-specific DDL clauses
-- (IDENTITY, system-versioning, MEMORY_OPTIMIZED, partition schemes) translated or
-- flagged below rather than silently dropped.

CREATE TABLE IF NOT EXISTS wwi_<env>.bronze.warehouse__coldroomtemperatures (
    ColdRoomTemperatureID BIGINT NOT NULL,
    ColdRoomSensorNumber INT NOT NULL,
    RecordedWhen TIMESTAMP NOT NULL,
    Temperature DECIMAL(10,2) NOT NULL,
    INDEX STRING COMMENT 'source type: [IX_Warehouse_ColdRoomTemperatures_ColdRoomSensorNumber]'
)
USING DELTA
COMMENT 'Converted from OLTP:Warehouse.ColdRoomTemperatures'
TBLPROPERTIES (
  'delta.enableChangeDataFeed' = 'true'
);

-- Conversion notes:
-- Column `ColdRoomTemperatureID` used IDENTITY/NEXT VALUE FOR in the source — replace with a Delta
...
-- Column `INDEX` ([IX_Warehouse_ColdRoomTemperatures_ColdRoomSensorNumber] -> STRING): Unrecognised
-- SQL Server type '[IX_Warehouse_ColdRoomTemperatures_ColdRoomSensorNumber]' — defaulted to STRING.
