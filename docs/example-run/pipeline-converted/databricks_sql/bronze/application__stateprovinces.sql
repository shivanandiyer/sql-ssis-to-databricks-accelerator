-- Source: OLTP:Application.StateProvinces  (/Users/shivaiyer/Documents/Claude/sql-server-samples/samples/databases/wide-world-importers/wwi-ssdt/wwi-ssdt/Application/Tables/StateProvinces.sql)
-- Converted: TABLE -> Delta table. Semantics preserved; SQL Server-specific DDL clauses
-- (IDENTITY, system-versioning, MEMORY_OPTIMIZED, partition schemes) translated or
-- flagged below rather than silently dropped.

CREATE TABLE IF NOT EXISTS wwi_<env>.bronze.application__stateprovinces (
...
USING DELTA
COMMENT 'Converted from OLTP:Application.StateProvinces'
TBLPROPERTIES (
  'delta.enableChangeDataFeed' = 'true'
);

-- Conversion notes:
-- Column `StateProvinceID` used IDENTITY/NEXT VALUE FOR in the source — replace with a Delta
...
-- [FK_Application_StateProvinces_Application_People] FOREIGN KEY ([LastEditedBy]) REFERENCES
-- [Application].[Peo
-- Foreign key not enforced by Delta Lake (informational only): CONSTRAINT
-- [FK_Application_StateProvinces_CountryID_Application_Countries] FOREIGN KEY ([CountryID]) REFERENCES
-- [Applica
