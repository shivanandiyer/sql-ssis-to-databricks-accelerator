-- Source: OLTP:Application.TransactionTypes  (/Users/shivaiyer/Documents/Claude/sql-server-samples/samples/databases/wide-world-importers/wwi-ssdt/wwi-ssdt/Application/Tables/TransactionTypes.sql)
-- Converted: TABLE -> Delta table. Semantics preserved; SQL Server-specific DDL clauses
-- (IDENTITY, system-versioning, MEMORY_OPTIMIZED, partition schemes) translated or
-- flagged below rather than silently dropped.

CREATE TABLE IF NOT EXISTS wwi_<env>.bronze.application__transactiontypes (
    TransactionTypeID INT NOT NULL,
    TransactionTypeName STRING NOT NULL,
    LastEditedBy INT NOT NULL
)
USING DELTA
COMMENT 'Converted from OLTP:Application.TransactionTypes'
TBLPROPERTIES (
  'delta.enableChangeDataFeed' = 'true'
);

-- Conversion notes:
-- Column `TransactionTypeID` used IDENTITY/NEXT VALUE FOR in the source — replace with a Delta
...
-- [FK_Application_TransactionTypes_Application_People] FOREIGN KEY ([LastEditedBy]) REFERENCES
-- [Application].[P
