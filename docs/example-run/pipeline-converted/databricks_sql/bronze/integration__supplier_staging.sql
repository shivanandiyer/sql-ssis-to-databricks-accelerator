-- Source: DW:Integration.Supplier_Staging  (/Users/shivaiyer/Documents/Claude/sql-server-samples/samples/databases/wide-world-importers/wwi-dw-ssdt/wwi-dw-ssdt/Integration/Tables/Supplier_Staging.sql)
-- Converted: TABLE -> Delta table. Semantics preserved; SQL Server-specific DDL clauses
-- (IDENTITY, system-versioning, MEMORY_OPTIMIZED, partition schemes) translated or
-- flagged below rather than silently dropped.

CREATE TABLE IF NOT EXISTS wwi_<env>.bronze.supplier_staging (
    Supplier STRING NOT NULL COMMENT 'source type: Staging',
...
USING DELTA
COMMENT 'Converted from DW:Integration.Supplier_Staging'
TBLPROPERTIES (
  'delta.enableChangeDataFeed' = 'true'
);

-- Conversion notes:
-- Column `Supplier` (Staging -> STRING): Unrecognised SQL Server type 'Staging' — defaulted to STRING.
