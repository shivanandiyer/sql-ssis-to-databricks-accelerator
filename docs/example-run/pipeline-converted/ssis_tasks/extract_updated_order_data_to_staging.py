# Converted from SSIS Data Flow Task: Extract Updated Order Data to Staging
...
def run(entity: str, last_cutoff: str, new_cutoff: str, environment: str, catalog: str) -> None:
    ...
    df = (
        ...
        .option("query", 'EXEC Integration.GetOrderUpdates ?, ?\nWITH RESULT SETS\n(\n    (\n        [Order Date Key] date,\n        [Picked Date Key] date,\n        [WWI Order ID] int,\n        [WWI Backorder ID] int,\n        [Description] nvarchar(100),\n        [Package] nvarchar(50),\n        Quantity int,\n        [Unit Price] decimal(18,2),\n        [Tax Rate] decimal(18,3),\n        [Total Excluding Tax] decimal(18,2),\n        [Tax Amount] decimal(18,2),\n        [Total Including Tax] decimal(18,2),\n        [WWI City ID] int,\n        [WWI Customer ID] int,\n        [WWI Stock Item ID] int,\n        [WWI Salesperson ID] int,\n        [WWI Picker ID] int,\n        [Last Modified When] datetime2(7)\n\n    )\n);' if False else "<TODO: parameterise with last_cutoff/new_cutoff>")
        .load()
    )
    target_fqn = f"{catalog}.bronze.integration_order_staging"
