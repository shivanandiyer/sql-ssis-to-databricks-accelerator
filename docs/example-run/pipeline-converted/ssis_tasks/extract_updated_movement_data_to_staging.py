# Converted from SSIS Data Flow Task: Extract Updated Movement Data to Staging
...
def run(entity: str, last_cutoff: str, new_cutoff: str, environment: str, catalog: str) -> None:
    ...
    df = (
        ...
        .option("query", 'EXEC Integration.GetMovementUpdates ?, ?\nWITH RESULT SETS\n(\n    (\n        [Date Key] date,\n        [WWI Stock Item Transaction ID] int,\n        [WWI Invoice ID] int,\n        [WWI Purchase Order ID] int,\n        [Quantity] int,\n        [WWI Stock Item ID] int,\n        [WWI Customer ID] int,\n        [WWI Supplier ID] int,\n        [WWI Transaction Type ID] int,\n        [Last Modified When] datetime2(7)\n    )\n);' if False else "<TODO: parameterise with last_cutoff/new_cutoff>")
        .load()
    )
    target_fqn = f"{catalog}.bronze.integration_movement_staging"
